...
namespace AppCommand
{
    public partial class FrmXuatKho : Form
    {
        ...
        {
            string command = "Select XK.SoPhieu [Bill_ID], DH.NgayDatHang [Ordered_At], XK.NgayXuatKho [Delivery_At],DH.NgayNhanHang [Arrived_At], DH.PT_ThanhToan [Paid via]";
            command += "From PhieuDatHang DH ";
            command += "Left Join PhieuXuatKho XK ";
            command += "On DH.SoPhieu = XK.SoPhieuDat ";
        ...
        {
            FrmXuatKho reload = new FrmXuatKho(user);
            this.Hide();
            reload.ShowDialog();
            this.Close();
        }
